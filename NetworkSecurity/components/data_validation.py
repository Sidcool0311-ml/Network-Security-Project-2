...
class DataValidation:
    ...
    def validate_no_of_columns(self,dataframe=pd.DataFrame):
        try:
            ...
            logging.info(f"total no of required columns{number_of_columns}")
            logging.info(f"total columns in dataframe {len(dataframe.columns)}")
    ...
    def detect_drift_report(self,base_df,current_df,threshold=0.05):
        try:
            status=True
            report={}
            for column in base_df.columns:
                ...
                if threshold<=sample_dist.pvalue:
                    ...
                    is_found=True
                    status=False
                report.update({column:{"p_value":float(sample_dist.pvalue),
                                       "drift_status":is_found}})
            drift_report_file_path=self.data_validation_config.drift_report_file_path
            ##making a directory
           # os.makedirs(dir_path=os.path.dirname(drift_report_file_path),exist_ok=True)
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            ##read data from train and test csvfiles
            ...
            ##checking the drift report
            drift_status=self.detect_drift_report(base_df=train_dataframe,current_df=test_dataframe)
            logging.info(f"drift report status{drift_status}")
